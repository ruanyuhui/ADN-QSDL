...
class ADNTrain(BaseTrain):
    ...
    def optimize(self, img_low, img_high):
        ...
        if self._nonzero_weight("gl", "lh", "ll"):
            ...
            ###loss gl
            nmi = mr.mutual_info_score(np.reshape(self.img_low, -1), np.reshape(self.img_high, -1))
            self.model_g._criterion["gl"](nmi*self.pred_lh,nmi*self.img_high)###这两个nmi自己加的，是低剂量的gan损失
        ...
        if self._nonzero_weight("hlh"):
            ...
            ###loss hlh
            nmi = mr.normalized_mutual_info_score(np.reshape(self.img_low, -1), np.reshape(self.img_high, -1))
            self.model_g._criterion["hlh"]( nmi*self.pred_hlh, nmi*self.img_high)###nmi注释2
        ...
        if self._nonzero_weight("art"):
            ...
            ###loss art
            nmi = mr.normalized_mutual_info_score(np.reshape(self.img_low, -1), np.reshape(self.img_high, -1))
            self.model_g._criterion["art"](nmi*(ll - self.pred_lh), nmi* (self.pred_hl - hh))###注释3个nmi（3）
